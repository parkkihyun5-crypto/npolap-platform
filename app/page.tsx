...
const consultingServices: ConsultingServiceType[] = [
  {
    id: "establish",
    title: "법인 설립 컨설팅",
...
const establishmentTypes: EstablishmentItemType[] = [
  {
    id: "voluntary",
    name: "비영리 임의단체",
    legal: "자율적 모임",
...
const establishmentPackages: PackageItemType[] = [
  {
    id: "ngo-basic",
    name: "임의단체 설립 패키지",
...
const brandingPackages: PackageItemType[] = [
  {
    id: "starter",
    name: "STARTER",
    subtitle: "BI Lite",
...
const brandingAddons: AddonItemType[] = [
...
function SectionHeader({
  ...
  return (
    <div className={center ? "mx-auto max-w-3xl text-center" : "max-w-3xl"}>
      <div className="text-sm font-semibold uppercase tracking-[0.22em]" style={{ color: accent }}>
        {eyebrow}
      </div>
...
function PackageCard({ item, active, onClick, theme }: PackageCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {item.popular ? (
          <span
            className="rounded-full px-3 py-1 text-xs font-bold text-[#0A1F33]"
      ...
      <div className="mt-6 text-2xl font-bold" style={{ color: theme.primary }}>
...
function LoginModal({ open, onClose, onSuccess, theme }: LoginModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 px-4">
      <div className="w-full max-w-md rounded-[28px] bg-white p-8 shadow-2xl">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-500">
          홈페이지 문구와 스타일을 수정하려면 관리자 로그인이 필요합니다.
        </p>
...
export default function PublicInterestConsultingPremiumSite() {
  ...
  const [selectedEstablishmentType, setSelectedEstablishmentType] =
    useState<string>("association");
  const [selectedFoundationPackage, setSelectedFoundationPackage] =
    useState<string>("association-pro");
  const [selectedBrandingPackage, setSelectedBrandingPackage] =
  ...
  const theme: ThemeConfigType = useMemo(
    () => ({
  ...
  const establishmentType = establishmentTypes.find(
    (item) => item.id === selectedEstablishmentType
  );
  const foundationPkg = establishmentPackages.find(
    (item) => item.id === selectedFoundationPackage
  );
  const brandingPkg = brandingPackages.find(
    (item) => item.id === selectedBrandingPackage
  );
  ...
  const submitInquiry = (e: React.FormEvent<HTMLFormElement>): void => {
    e.preventDefault();
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 md:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <a
              href="#establishment"
              className="hidden rounded-lg bg-slate-100 px-4 py-2 text-sm font-semibold text-slate-700 md:block"
            >
              {content.topMenu1}
            </a>
            <a
      ...
      <main>
        ...
        >
          <div className="relative mx-auto max-w-7xl px-6">
            ...
            <div className="mt-8 flex gap-4">
              <a
                href="#establishment"
                className="rounded-xl px-6 py-3 font-bold text-slate-900"
                style={{ backgroundColor: theme.accent }}
              >
                {content.primaryCta}
              </a>
        ...
        <section className="mx-auto max-w-7xl px-6 py-20 md:px-10 lg:px-12">
          <SectionHeader
            ...
            desc={content.consultingDescription}
            center
            accent={theme.accent}
          />
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {consultingServices.map((item) => (
              <div
                key={item.id}
                className="rounded-[24px] border border-slate-200 p-6 shadow-sm"
        ...
        <section
          id="establishment"
          className="border-y border-slate-200 py-20"
          style={{ backgroundColor: theme.cardBg }}
        >
          <div className="mx-auto max-w-7xl px-6 md:px-10 lg:px-12">
            <SectionHeader
              eyebrow="Establishment"
              title={content.establishmentTitle}
              desc={content.establishmentDescription}
              center
              accent={theme.accent}
            />
            ...
            <div className="mt-12 grid gap-6 lg:grid-cols-4">
              {establishmentTypes.map((item) => (
                <button
                  ...
                  className={`rounded-[24px] border p-6 text-left shadow-sm transition hover:-translate-y-1 hover:shadow-xl ${
                    selectedEstablishmentType === item.id ? "ring-2 ring-slate-200" : ""
                  }`}
                  style={{
                    backgroundColor: theme.cardBg,
                    borderColor:
                      selectedEstablishmentType === item.id ? theme.accent : "#E2E8F0",
                  }}
                >
                  <div className="text-sm font-semibold" style={{ color: theme.accent }}>
                    {item.legal}
                  ...
                  <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-xl bg-slate-50 p-3">
                      <div className="text-xs text-slate-500">절차</div>
                      <div className="mt-1 font-semibold">{item.procedure}</div>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3">
                      <div className="text-xs text-slate-500">난이도</div>
                      <div className="mt-1 font-semibold">{item.difficulty}</div>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3">
                      <div className="text-xs text-slate-500">기간</div>
                      <div className="mt-1 font-semibold">{item.period}</div>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3">
                      <div className="text-xs text-slate-500">비용</div>
                      <div className="mt-1 font-semibold">{item.budget}</div>
                    </div>
                  </div>
                </button>
              ))}
            ...
            <div className="mt-12 grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
              ...
              <div className="space-y-6">
                <div
                  className="rounded-[28px] p-6 text-white shadow-2xl"
                  style={{ backgroundColor: theme.primary }}
                >
                  <div
                    className="text-sm font-semibold uppercase tracking-[0.2em]"
                    ...
                    {establishmentPackages.map((item) => (
                      <button
                        ...
                        className={`w-full rounded-2xl border p-4 text-left transition ${
                          selectedFoundationPackage === item.id
                ...
                >
                  ...
                  <p className="mt-4 text-sm leading-7 text-slate-600">
                    사무국은 정관을 실제로 작동시키는 엔진입니다. 준비 없이 제출하면
                    보완 요구, 재작성 비용, 일정 지연, 파트너십 기회 상실로 이어질 수
                    있으므로 초기 설계와 실무 준비가 매우 중요합니다.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section id="branding" className="py-20">
          <div className="mx-auto max-w-7xl px-6 md:px-10 lg:px-12">
            <SectionHeader
              ...
              desc={content.brandingDescription}
              center
              accent={theme.accent}
            />
            ...
            <div className="mt-12 grid gap-6 lg:grid-cols-4">
              {brandingPackages.map((item) => (
                <PackageCard
                  key={item.id}
                  item={item}
                  active={selectedBrandingPackage === item.id}
                  onClick={() => setSelectedBrandingPackage(item.id)}
                  theme={theme}
                />
              ))}
            </div>
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-2xl border p-5 text-left transition ${
                          checked ? "text-white shadow-lg" : "border-slate-200 bg-white"
                        }`}
                        style={
                          checked
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <div
                            ...
                            style={checked ? { color: theme.primary } : {}}
                          >
                            {checked ? "✓" : "+"}
                          </div>
                        </div>
                    ...
                    {directions.map((item) => (
                      <button
                        ...
                        className={`rounded-xl border px-4 py-3 text-left text-sm font-semibold ${
                          selectedDirection === item.id ? "shadow-sm" : "border-slate-200"
                        }`}
                        style={
                          selectedDirection === item.id
                            ? { borderColor: theme.accent, backgroundColor: "#FFF8ED" }
                            : {}
                        }
                      >
                        {item.label}
                ...
                >
                  ...
                  <div className="mt-6 rounded-2xl bg-white/5 p-5">
                    <div className="flex items-center justify-between text-sm text-slate-300">
                      <span>설립 패키지</span>
                      <span className="font-semibold text-white">{foundationPkg?.name}</span>
                    </div>
                    <div className="mt-3 flex items-center justify-between text-sm text-slate-300">
                      <span>브랜딩 패키지</span>
                      <span className="font-semibold text-white">{brandingPkg?.name}</span>
                    </div>
                    <div className="mt-3 flex items-center justify-between text-sm text-slate-300">
                      <span>브랜딩 옵션</span>
                      <span className="font-semibold text-white">
                        {selectedAddons.length}개
                      </span>
                    </div>
                    <div className="mt-3 flex items-center justify-between text-sm text-slate-300">
                      <span>브랜드 방향</span>
                    ...
                    <div className="mt-4 border-t border-white/10 pt-4">
                      <div className="text-sm text-slate-300">
                        총 견적{vatIncluded ? " (VAT 포함)" : ""}
                      </div>
                      <div className="mt-2 text-4xl font-bold" style={{ color: theme.accent }}>
                        {formatKRW(total)}
                      </div>
                    </div>
                  </div>
                  ...
                  <a
                    href="#contact"
                    className="mt-6 block w-full rounded-2xl px-5 py-4 text-center font-bold text-[#0A1F33]"
                    style={{ backgroundColor: theme.accent }}
                  >
                    이 구성으로 상담 신청
                  </a>
            ...
            <div className="grid gap-8 lg:grid-cols-2">
              <div className="rounded-[28px] border border-slate-200 p-6 shadow-sm">
                <SectionHeader
                  ...
                  desc={content.paymentDescription}
                  accent={theme.accent}
                />
                ...
                <div className="mt-8 rounded-2xl bg-slate-50 p-5">
                  <div className="grid gap-4 md:grid-cols-3">
                    <div>
                      <div className="text-xs font-semibold text-slate-500">은행</div>
                      <div className="mt-1 text-lg font-bold">{content.bankName}</div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-slate-500">계좌번호</div>
                      <div className="mt-1 text-lg font-bold">{content.bankAccount}</div>
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-slate-500">예금주</div>
                      <div className="mt-1 text-lg font-bold">{content.bankHolder}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  <button
                    type="button"
                    onClick={() => copyText(content.bankAccount)}
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      copyText(`${content.bankName} ${content.bankAccount} ${content.bankHolder}`)
                    }
              ...
              <div id="contact" className="rounded-[28px] border border-slate-200 p-6 shadow-sm">
                <SectionHeader
                  ...
                  desc={content.contactDescription}
                  accent={theme.accent}
                />
                ...
                <form onSubmit={submitInquiry} className="mt-6">
                  ...
                  <textarea
                    ...
                    rows={6}
                    placeholder="요청사항"
                    className="mt-5 w-full rounded-xl border border-slate-300 px-4 py-3 outline-none focus:border-slate-800"
                  />

                  <button
                    type="submit"
                    className="mt-6 w-full rounded-2xl px-6 py-4 text-lg font-bold text-white"
                    style={{ backgroundColor: theme.primary }}
                  >
                    상담 신청하기
                  </button>
          ...
          <section className="mx-auto max-w-7xl px-6 py-20 md:px-10 lg:px-12">
            <div className="rounded-[32px] border border-slate-200 bg-white p-6 shadow-xl md:p-8">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-500">
                    문구, 연락처, 입금계좌, 색상 테마를 수정하고 저장할 수 있습니다.
                  </p>
                </div>
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    type="button"
                    onClick={reset}
                    className="rounded-xl bg-slate-200 px-5 py-3 text-sm font-bold text-slate-800"
                  >
                    기본값 초기화
                  </button>
                </div>
              ...
              <div className="mt-8 grid gap-8 lg:grid-cols-2">
                ...
                <AdminField
                  label="사이트 부제"
                  value={content.siteSubtitle}
                  onChange={(v) => updateContent("siteSubtitle", v)}
                />
                <AdminField
                  label="상단 배지"
                  value={content.heroBadge}
                  onChange={(v) => updateContent("heroBadge", v)}
                />
                <AdminField
                  label="연락처"
                  value={content.phone}
                ...
                <AdminField
                  ...
                  onChange={(v) => updateContent("bankName", v)}
                />
                <AdminField
                  label="계좌번호"
                  value={content.bankAccount}
                  onChange={(v) => updateContent("bankAccount", v)}
                />
                <AdminField
                  label="예금주"
                  value={content.bankHolder}
                  onChange={(v) => updateContent("bankHolder", v)}
                />
                ...
                <AdminField
                  label="설립유형 섹션 제목"
                  value={content.establishmentTitle}
                  onChange={(v) => updateContent("establishmentTitle", v)}
                />
                <AdminField
                  label="브랜딩 섹션 제목"
                  value={content.brandingTitle}
                  onChange={(v) => updateContent("brandingTitle", v)}
                />
                <AdminField
                  label="입금 섹션 제목"
                  value={content.paymentTitle}
                  onChange={(v) => updateContent("paymentTitle", v)}
                />
                <AdminField
                  label="상담 섹션 제목"
                  value={content.contactTitle}
                  onChange={(v) => updateContent("contactTitle", v)}
                />

                <AdminField
                  label="기본색"
                  value={content.primary}
                  onChange={(v) => updateContent("primary", v)}
                  color
                />
                <AdminField
                  label="보조색"
                  value={content.secondary}
                  onChange={(v) => updateContent("secondary", v)}
                  color
                />
                <AdminField
                  label="강조색"
                  value={content.accent}
                  onChange={(v) => updateContent("accent", v)}
                  color
                />
                <AdminField
                  label="배경색"
                  value={content.bodyBg}
                  onChange={(v) => updateContent("bodyBg", v)}
                  color
                />
              </div>
            </div>
      ...
      <footer className="border-t border-slate-200 bg-white/70">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-6 py-8 text-sm text-slate-600 md:flex-row md:items-center md:justify-between md:px-10 lg:px-12">
          ...
          <div className="flex flex-wrap gap-4">
            <span>
              {content.bankName} {content.bankAccount}
            </span>
